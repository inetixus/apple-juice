      ...
      if (account?.provider === "google") {
        ...
        if (email) {
          try {
            ...
            if (!agMapping) {
              const { linkAntigravityAccount } =
                await import("@/lib/antigravity");
              agMapping = await linkAntigravityAccount(email, email); // use email as the ID
            }
            if (agMapping) {
              (token as { antigravityId?: string }).antigravityId =
                agMapping.antigravityUserId;
            }
          } catch (err) {
            console.warn(
              "Antigravity mapping lookup failed:",
