 ...
 */
export async function unlinkAntigravityAccount(
  googleEmail: string,
): Promise<void> {
  const redis = getRedis();
  const key = `${AG_MAP_PREFIX}${googleEmail.toLowerCase()}`;
...
export async function checkAntigravityBalance(
  ...
  return {
    quotas: [
      {
        model: "📊 Requests This Week",
        refreshesIn: formatNum(usage.requests),
      },
...
function resolveModelId(displayName: string): string {
  ...
  if (displayName.startsWith("models/"))
    return displayName.replace("models/", "");
  ...
  if (displayName.toLowerCase().includes("deepseek")) {
    if (displayName.toLowerCase().includes("r1"))
      return "deepseek-ai/deepseek-r1";
    return "deepseek-ai/deepseek-v3.2-maas";
  }
  // Default
...
async function relayToVertexDeepSeek(
  ...
  // DeepSeek V3 and R1 on Vertex AI MaaS
  // Currently both work best in the 'global' region with the openapi endpoint.
  const region = "global";
  const modelId = isR1
    ? "deepseek-ai/deepseek-r1"
    : "deepseek-ai/deepseek-v3.2-maas";
  const baseUrl = "aiplatform.googleapis.com";
  ...
  try {
    ...
    if (!keyPath && !keyJson) {
      throw new Error(
        "Missing GOOGLE_SERVICE_ACCOUNT_PATH or GOOGLE_SERVICE_ACCOUNT_JSON in environment.",
      );
    }
    ...
    // Shared OpenAI-compatible endpoint (Matches test-deepseek.mjs)
    ...
    const payload = {
      ...
      max_tokens: Math.min(request.max_tokens ?? 32768, 32768),
      stream: !!request.stream,
    };

    console.log(
      `[DeepSeek] Calling ${modelId} at ${url}${request.stream ? " (STREAMING)" : ""}...`,
    );
    ...
    if (!res.ok) {
      ...
      try {
        ...
        if (res.status === 429) {
          ...
          errorMessage =
            "Authentication failed. Please log out and log back in with Google.";
        } else {
