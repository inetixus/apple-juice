...
1. ARCHITECTURE: Always specify multi-script architecture — ServerScript for logic, ModuleScript for shared data/config, LocalScript for client UI/input. Specify exact parent locations (ServerScriptService, ReplicatedStorage, StarterGui, etc.). DEMAND self-contained scripts that create their own dependencies (ScreenGuis, Folders, RemoteEvents) if they are missing, rather than yielding forever with WaitForChild.
...
Do NOT write code. ONLY output the rewritten, improved prompt as plain text. No conversational text. No quotes. No markdown formatting.`;
    ...
    if (effectiveProvider === "google") {
      ...
      if (llmRes.ok) {
        ...
        improvedPrompt =
          parsed?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || prompt;
      }
    } else {
      ...
      if (res.ok) {
        ...
        improvedPrompt =
          parsed?.choices?.[0]?.message?.content?.trim() || prompt;
      }
    }
